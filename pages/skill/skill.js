...
const unavailableSkillIds = ['paper-outline', 'english-translate', 'ppt-outline', 'meeting-summary', 'email-polish', 'weekly-report'];
...
Page({
  ...
  handleChooseTryOnImage(event) {
    ...
    wx.chooseMedia({
      ...
      success: res => {
        ...
        this.setData({
          [`tryOnImages.${field}`]: file.tempFilePath
        });
      }
    });
  },
  ...
  buildImagePrompt() {
    ...
    if (this.data.isSellingPoints) {
      ...
      return [
        ...
        `产品图片/包装图/详情页图：${this.data.uploadedImage ? '已上传' : '未上传'}`,
    ...
    if (this.data.isTryOn) {
      return [
        ...
        `衣服照片：${this.data.tryOnImages.clothing ? '已上传' : '未上传'}`,
        `本人照片：${this.data.tryOnImages.person ? '已上传' : '未上传'}`,
  ...
  handleGenerate() {
    ...
    if (this.data.isHotspotTracker) {
      if (!this.data.marketingForm.industry.trim()) {
        wx.showToast({
          title: '请填写行业',
          icon: 'none'
        });
      ...
      if (!this.data.marketingForm.category.trim()) {
        wx.showToast({
          title: '请填写品类',
          icon: 'none'
        });
    ...
    if (this.data.isCommunityBroadcast && !this.data.marketingForm.brandName.trim()) {
      wx.showToast({
        title: '请填写品牌名称',
        icon: 'none'
      });
    ...
    if (this.data.isDianpingImage) {
      ...
      if (!this.data.marketingForm.brandName.trim()) {
        wx.showToast({
          title: '请填写品牌名称',
          icon: 'none'
        });
    ...
    if (this.data.isTryOn) {
      if (!this.data.tryOnImages.clothing) {
        wx.showToast({
          title: '请上传衣服照片',
          icon: 'none'
        });
      ...
      if (!this.data.tryOnImages.person) {
        wx.showToast({
          title: '请上传本人照片',
          icon: 'none'
        });
    ...
    if (this.data.isImageSkill && !this.data.isTryOn && !this.data.uploadedImage) {
      wx.showToast({
        title: '请先上传图片',
        icon: 'none'
      });
  ...
  createGenerationTask(generationPrompt) {
    ...
    mock.createTask({
      skillId: this.data.skillId,
      prompt: generationPrompt,
      image: this.data.uploadedImage || this.data.tryOnImages.person || this.data.tryOnImages.clothing
    }).then(data => {
      setTimeout(() => {
  ...
  saveRecord(resultImage) {
    mock.saveRecord({
      ...
      inputImage: this.data.uploadedImage || this.data.tryOnImages.person || this.data.tryOnImages.clothing,
      resultImage
    });
  },
